...
WRAPPER_LIB="$SCRIPT_DIR/libswapcapture.so"
PYTHON_SCRIPT="$SCRIPT_DIR/upscale_openvino.py"
...
if [ ! -f "$WRAPPER_LIB" ]; then
    ...
    echo "Ejecuta: gcc -shared -fPIC -o libswapcapture.so wrapper_swapbuffers_shm.c -lGL -lX11 -ldl -lrt"
    exit 1
fi
